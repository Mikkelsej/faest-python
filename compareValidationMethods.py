...
from sudoku_validator import Check0Validator, PITValidator, SudokuValidator
...
def main():
    ...
    print(
        f"Check0 Method used {check0_stats[0]} fresh VOLEs + {check0_stats[1]} temp storage. Valid: {check0_valid}"
    )

    print(
        f"PIT Method used {pit_stats[0]} fresh VOLEs + {pit_stats[1]} temp storage. Valid: {pit_valid}"
    )
