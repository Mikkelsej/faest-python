...
def check_sequence(sequence, target_sq: int, target_cube: int, field: ExtensionField) -> bool:
...
def verify_sudoku_constraints(n: int, field_bits: int = 64) -> None:
    ...
    for combo in product(range(1, n + 1), repeat=n):
        ...
        if total_checked % 1_000_000 == 0:
            print(
                f"  Checked {total_checked:,} combinations... ({false_positives} false positives so far)"
            )
    ...
    if invalid_perms == 0 and false_positives == 0:
        ...
        if false_positives > 0:
            print(
                f"  {false_positives}/{non_perm_checked} invalid combinations passed (false positives)"
            )
        print(f"  Total combinations checked: {total_checked}")
